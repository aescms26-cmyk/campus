...
CREATE TABLE IF NOT EXISTS public.users (
  ...
  name TEXT,
  role TEXT CHECK (role IN ('admin', 'team_lead', 'counsellor')) NOT NULL,
...
-- Helper function to check if user is admin (using JWT metadata)
-- Note: This requires the 'role' to be in user_metadata
...
-- Enquiries: Admins, Team Leads (their team), Counsellors (assigned)
...
-- Time Logs: Admins, Team Leads (their team), Counsellors (their own)
CREATE POLICY "Admins can manage time logs" ON public.time_logs FOR ALL USING (
  (auth.jwt() -> 'user_metadata' ->> 'role') = 'admin'
);
